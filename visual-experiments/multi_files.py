...
class Visualizer:
    ...
    def draw_chunk(self, chunk):
        ...
        pan_x = (chunk.pan - 0.5)
        y = self.filenum_to_y_coord(chunk.filenum)
        y1 = int(y - 3 + 20 * pan_x * actuality)
        y2 = int(y + 3 + 20 * pan_x * actuality)
        x1 = int(self.byte_to_x_coord(chunk.begin))
        x2 = int(self.byte_to_x_coord(chunk.end))
        if x2 == x1:
            x2 = x1 + 1
    ...
    def update_x_scope(self):
        ...
        if diff == 0:
            ...
            self.x_ratio = float(self.width) / diff
