...
for (session_pattern, render_args, timefactors) in sessions:
    ...
    if os.path.exists(render_dir) and not args.force:
        ...
        cmd = "./revisualize_ancestry.py --geometry=circle %s %s %s --node-style=circle -interpret --sway --sway-magnitude=0.003 --edge-style=line --line-width=1 --node-size-envelope=0.3,25,0.5,0.2 --root-node-size-envelope=0.1,0,1,0.2 --sway-envelope=0,50,0.5,1 --node-size=0.003 --root-node-size=0.005 -export -export-fps=%s" % (session_dir, render_args, RESOLUTION, FPS)
